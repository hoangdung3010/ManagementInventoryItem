...
{
    public class Print_KH_XuatBaoDam_DetailRepository : BaseReportRepository
    {
        public Print_KH_XuatBaoDam_DetailRepository(IUnitOfWork unitOfWork):base(unitOfWork)
        {
        ...
        protected override void CustomInitParameterExcel(ConfigExcel configExcel, XlsFile result, FlexCelReport fr)
        {
            ...
            var piLoai = configExcel.ParametersExcel.GetValueOfDictionary("iLoaiKH");
            if (piLoai != null)
            {
                MTControl.DisplayEnum displayEnum = MTControl.MCommon.GetDisplayEnum("iLoaiKH",
                    Convert.ToInt32(piLoai.OriginValue));
                configExcel.ParametersExcel.AddOrUpdate("iLoaiKH_Text", new MTParameter
                {
                    Value = displayEnum?.Text,
            ...
            //SubTitle
            var pdNgayKH_XuatBaoDam = configExcel.ParametersExcel.GetValueOfDictionary("dNgayKeHoach");
            if (pdNgayKH_XuatBaoDam != null)
            {
                DateTime dNgayLap =(DateTime)pdNgayKH_XuatBaoDam.OriginValue;
