...
{
    public class Print_KH_XuatTHTH_DetailRepository : BaseReportRepository
    {
        public Print_KH_XuatTHTH_DetailRepository(IUnitOfWork unitOfWork):base(unitOfWork)
        {
        ...
        protected override void CustomInitParameterExcel(ConfigExcel configExcel, XlsFile result, FlexCelReport fr)
        {
           ...
           /* var piLoai = configExcel.ParametersExcel.GetValueOfDictionary("iTCXuat");
            ...
            //SubTitle
            var pdNgayKH_XuatTHTH = configExcel.ParametersExcel.GetValueOfDictionary("dNgayKeHoach");
            if (pdNgayKH_XuatTHTH != null)
            {
                DateTime dNgayLap =(DateTime)pdNgayKH_XuatTHTH.OriginValue;
