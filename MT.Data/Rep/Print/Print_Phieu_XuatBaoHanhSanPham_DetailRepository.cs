...
{
    public class Print_Phieu_XuatBaoHanhSanPham_DetailRepository : BaseReportRepository
    {
        public Print_Phieu_XuatBaoHanhSanPham_DetailRepository(IUnitOfWork unitOfWork):base(unitOfWork)
        {
        ...
        protected override void CustomInitParameterExcel(ConfigExcel configExcel, XlsFile result, FlexCelReport fr)
        {
            ...
            if (piLoai != null)
            {
                MTControl.DisplayEnum displayEnum = MTControl.MCommon.GetDisplayEnum("iLoaiXuatBaoHanh",
            ...
            //SubTitle
            var pdNgayPhieu_Xuat= configExcel.ParametersExcel.GetValueOfDictionary("dNgayPhieu_Xuat");
            if (pdNgayPhieu_Xuat != null)
            {
                DateTime dNgayPhieu_Xuat = (DateTime)pdNgayPhieu_Xuat.OriginValue;
                configExcel.SubTitle = $"Ngày {dNgayPhieu_Xuat.Day.ToString().PadLeft(2,'0')}, tháng {dNgayPhieu_Xuat.Month.ToString().PadLeft(2, '0')}, năm {dNgayPhieu_Xuat.Year}";
            }
            //Tham số phụ lục
            configExcel.ParametersExcel.AddOrUpdate("PL_Title", new MTParameter
            {
                Value = "PHỤ LỤC CHI TIẾT PHIẾU XUẤT BẢO HÀNH SẢN PHẨM"
            });
            ...
            {
                Value = $"Kèm theo phiếu xuất số {psMa.OriginValue}"
            });
        }
