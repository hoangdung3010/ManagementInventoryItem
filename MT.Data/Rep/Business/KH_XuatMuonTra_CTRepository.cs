...
{
    public class KH_XuatMuonTra_CTRepository : MT.Data.Rep.BaseRepository<KH_XuatMuonTra_CT>
    {
        #region "Constructor"

        public KH_XuatMuonTra_CTRepository(IUnitOfWork unitOfWork) : base(unitOfWork)
        {
        }
        ...
        /// <returns></returns>
        public KH_XuatMuonTra_CT GetKH_XuatMuonTra_CTBysKH_XuatMuonTra_IdAndsDM_SanPham_Id(Guid sKH_XuatMuonTra_Id,
            Guid sDM_SanPham_Id, Guid? sDM_DonViTinh_Id, Guid? sKH_XuatMuonTra_CT_Id = null)
        {
            string query = $@"select KHBD_CT.Id, KHBD_CT.sDM_SanPham_Id,SP.sTenSanPham as sDM_SanPham_Id_Ten,
                                ...
                                from [dbo].KH_XuatMuonTra_CT KHBD_CT
                                JOIN dbo.DM_SanPham SP ON KHBD_CT.sDM_SanPham_Id=SP.ID
                                LEFT JOIN  (SELECT sKH_XuatMuonTra_CT_Id,sum(rSoLuong) as rSoLuong FROM
								[dbo].[KH_XuatMuonTra_CT]	group by sKH_XuatMuonTra_CT_Id) as KHX_CT ON KHBD_CT.Id=KHX_CT.sKH_XuatMuonTra_CT_Id
                            WHERE KHBD_CT.sKH_XuatMuonTra_Id='{sKH_XuatMuonTra_Id}'
                            AND KHBD_CT.sDM_SanPham_Id='{sDM_SanPham_Id}'";
            if (sKH_XuatMuonTra_CT_Id.HasValue)
            {
                query += $" AND KHBD_CT.Id!='{sKH_XuatMuonTra_CT_Id}'";
            }
            if (sDM_DonViTinh_Id.HasValue)
            {
                query += $" AND KHBD_CT.sDM_DonViTinh_Id='{sDM_DonViTinh_Id}'";
            }
            return _unitOfWork.QueryFirstOrDefault<KH_XuatMuonTra_CT>(query);
        }

        public KH_XuatMuonTra_CT GetKH_XuatMuonTra_CTBysKH_XuatMuonTra_Id(Guid sKH_XuatMuonTra_Id)
        {
            string query = $@"select KHBD_CT.Id, KHBD_CT.sDM_SanPham_Id,SP.sTenSanPham as sDM_SanPham_Id_Ten,
                                KHBD_CT.rSoLuong - ISNULL(KHX_CT.rSoLuong,0) as rSoLuong,
                                ISNULL(KHBD_CT.iThoiGianBaoHanh,60) as iThoiGianBaoHanh,ISNULL(KHBD_CT.iDonViThoiGianBaoHanh,1) as iDonViThoiGianBaoHanh,
        ...
        /// <returns></returns>
        public KH_XuatMuonTra_CT GetKH_XuatMuonTra_CTsBysKH_XuatMuonTra_IdAndsIDSP(Guid sKH_XuatMuonTra_Id, string sSPID)
        {
            string query = $@"SELECT KH_CT.Id ,KH_CT.rSoLuong,KH_CT.rDonGia,KH_CT.sDM_DonViTinh_Id,KH_CT.sDM_SanPham_Id
                              FROM dbo.KH_XuatMuonTra_CT  KH_CT
                              WHERE KH_CT.sKH_XuatMuonTra_Id='{sKH_XuatMuonTra_Id}' AND KH_CT.sDM_SanPham_Id='{sSPID}'";

            return _unitOfWork.QueryFirstOrDefault<KH_XuatMuonTra_CT>(query, new { sDM_SanPham_Id = "'" + sSPID + "'" });
        }
        ...
        public IList GetKHXuatMuonTra_CTsByMasterId(Guid masterId)
        {
            ...
            KHXBD_CT.Id as sKH_XuatMuonTra_CT_Id,
            ...
            from [dbo].KH_XuatMuonTra_CT KHXBD_CT
            JOIN dbo.DM_SanPham SP ON KHXBD_CT.sDM_SanPham_Id=SP.ID
            LEFT JOIN  (SELECT sKH_XuatMuonTra_CT_Id,sum(rSoLuong) as rSoLuong FROM
            [dbo].[KH_XuatMuonTra_CT]	group by sKH_XuatMuonTra_CT_Id) AS KHX_CT
            ON KHXBD_CT.Id=KHX_CT.sKH_XuatMuonTra_CT_Id
            WHERE KHXBD_CT.sKH_XuatMuonTra_Id='{masterId}'
            AND KHXBD_CT.rSoLuong - ISNULL(KHX_CT.rSoLuong,0) >0
            ORDER BY KHXBD_CT.SortOrder";
            List<KH_XuatMuonTra_CT> kHXuatMuonTra_CTs = _unitOfWork.Query<KH_XuatMuonTra_CT>(query);
            if (kHXuatMuonTra_CTs != null)
            {
                foreach (var item in kHXuatMuonTra_CTs)
                {
                    item.MTEntityState = Enummation.MTEntityState.Add;
                }
            }

            return kHXuatMuonTra_CTs;
        }
        ...
        /// <param name="masterId">Id của kế hoạch nhập mới</param>
        /// <returns>Danh sách chi tiết kế hoạch</returns>
        public IList<DM_DonVi> GetListDonViNhanInKeHoachMuonTraID(Guid KHBDId)
        {
            ...
            WHERE KHXBD_CT.sKH_XuatMuonTra_Id='{KHBDId}'
        ...
        /// <param name="masterId">Id của kế hoạch nhập mới</param>
        /// <returns>Danh sách chi tiết kế hoạch</returns>
        public IList GetKHXuatMuonTra_CTsByMasterIdAndDonviNhap(Guid masterId, Guid donviNhapId)
        {
            string query = $@"select NewId() as Id,
            KHXBD_CT.Id as sKH_XuatMuonTra_CT_Id,
            KHXBD_CT.sDM_SanPham_Id,SP.sTenSanPham as sDM_SanPham_Id_Ten,
        ...
        /// </summary>
        /// <param name="sKH_XuatMuonTra_Id">ID của kế hoạch nhập mới</param>
        /// <returns></returns>
        public List<DM_SanPham> GetKH_XuaMuonTra_CTsBysKH_XuatMuonTra_Id(Guid sKH_XuatMuonTra_Id)
        {
            string query = $@"SELECT SP.Id,SP.sMaSanPham,SP.sTenSanPham FROM dbo.KH_XuatMuonTra_CT  KHBD_CT
                              JOIN dbo.DM_SanPham SP ON KHBD_CT.sDM_SanPham_Id=SP.Id
                              WHERE sKH_XuatMuonTra_Id='{sKH_XuatMuonTra_Id}' ORDER BY KHBD_CT.SortOrder ";
        ...
        public KH_XuatMuonTra_CT GetKH_XuatMuonTra_CTBysKH_XuatMuonTra_IdAndsMaSP(Guid? sKH_XuatMuonTra_Id, string sMaSP)
        {
            // Note 14.6
            /*string query = $@"SELECT KH_CT.Id ,SP.sMaSanPham,SP.sTenSanPham as sDM_SanPham_Id_Ten,KH_CT.rSoLuong,KH_CT.rDonGia,KH_CT.sDM_DonViTinh_Id,KH_CT.sDM_SanPham_Id
                              ...
                              WHERE KH_CT.sKH_XuatMuonTra_Id='{sKH_XuatMuonTra_Id}' AND SP.sMaSanPham=@sMaSanPham";*/
            string query = $@"SELECT *
                                FROM dbo.View_KH_XuatMuonTra_CT  KH_CT
                              JOIN dbo.DM_SanPham SP ON KH_CT.sDM_SanPham_Id=SP.Id
                              WHERE KH_CT.sKH_XuatMuonTra_Id='{sKH_XuatMuonTra_Id}' AND SP.sMaSanPham=@sMaSanPham";

            return _unitOfWork.QueryFirstOrDefault<KH_XuatMuonTra_CT>(query, new { sMaSanPham = sMaSP });
        }
    }
}