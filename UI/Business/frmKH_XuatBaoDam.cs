...
namespace FormUI
{
    public partial class frmKH_XuatBaoDam : FormUI.MFormList
    {
        #region"Property"
        ...
        private void InitGrid()
        {
            ...
            grd.ViewName = ((KH_XuatBaoDamRepository)ConfigFormDetail.Rep).GetViewPaging();
            ...
            this.gridMaster.AddColumnText("dNgayKeHoach", "Ngày kế hoạch", 100, fixedStyle: DevExpress.XtraGrid.Columns.FixedStyle.Left);
            ...
            colMaster.FilterEditor = new ConfigFilterEditor { DataType = DataTypeColumn.ComboBox };
            //Sản phẩm
            grdSanPham.TableName = "KH_XuatBaoDam_CT";
            grdSanPham.ViewName = "View_KH_XuatBaoDam_CT";
            ...
            col = grdSanPham.AddColumnText("sDM_HangMucBaoDam_Id_Ten", "Loại hạng mục", 180, isFixWidth: true);
            ...
            grdPhuKienDetail.TableName = "KH_XuatBaoDam_CT_PhuKien";
            grdPhuKienDetail.ViewName = "View_KH_XuatBaoDam_CT_PhuKien";
        ...
        {
            var datas = gridMaster.GetListRecordByRowsSelected<MT.Data.BO.KH_XuatBaoDam>();

            if (datas == null || datas.Count == 0)
            {
                MMessage.ShowWarning("Bạn phải chọn ít nhất 1 kế hoạch bảo đảm.");
                return false;
            }
            ...
            foreach (var item in datas)
            {
                ...
                {
                    MMessage.ShowWarning($"Bạn không có quyền {msg} với kế hoạch bảo đảm <{item.sMa}>.");
                    return false;
                }

                //Nếu Kế hoạch đã phát sinh phiếu nhập thì không cho phế duyệt hoặc từ chối
                //Nếu kế hoạch bảo đảm đã phát sinh kế hoạch xuất thì không cho sửa
                ...
                {
                    MMessage.ShowWarning($"Bạn không được phép {msg}. Do kế hoạch bảo đảm <{item.sMa}> đã phát sinh các kế hoạch xuất <{string.Join(", ", kHXuatSP_sMas)}>.");
                    return false;
                }
            }
        ...
        protected override void ProcessItemClick(int tag, object sender)
        {
            ...
            switch (tag)
            {
                ...
                //    var data = gridMaster.GetRecordByRowSelected<MT.Data.BO.KH_XuatBaoDam>();
                //    frmKH_XuatBaoDam_PheDuyet frmKH_XuatBaoDam_PheDuyet = new frmKH_XuatBaoDam_PheDuyet(data);
                //    frmKH_XuatBaoDam_PheDuyet.ShowDialog();
                //    frmKH_XuatBaoDam_PheDuyet.Dispose();
                //    break;
                ...
                case (int)MCommandName.Approve:
                    ...
                    {
                        var datasApprove = gridMaster.GetListRecordByRowsSelected<MT.Data.BO.KH_XuatBaoDam>();
                        if (datasApprove.Any(m => m.iTrangThaiDuyet == (int)MT.Data.iTrangThaiDuyetPNM.DongYDuyet))
                        {
                            ...
                            MMessage.ShowWarning($"Các kế hoạch bảo đảm <{strMaApproves}> đã được duyệt. Bạn vui lòng chọn kế hoạch khác.");
                            return;
                        }

                        var frmKH_XuatBaoDam_PheDuyet = new frmKH_XuatBaoDam_PheDuyet(this.RefreshData, (int)MT.Data.iTrangThaiDuyetPNM.DongYDuyet, datasApprove);
                        frmKH_XuatBaoDam_PheDuyet.ShowDialog();
                        frmKH_XuatBaoDam_PheDuyet.Dispose();
                    }
                    break;
                ...
                case (int)MCommandName.Reject:
                    ...
                    {
                        var datasReject = gridMaster.GetListRecordByRowsSelected<MT.Data.BO.KH_XuatBaoDam>();
                        if (datasReject.Any(m => m.iTrangThaiDuyet == (int)MT.Data.iTrangThaiDuyetPNM.TuChoiDuyet))
                        {
                            ...
                            MMessage.ShowWarning($"Các kế hoạch bảo đảm <{strMaRejects}> đã bị từ chối. Bạn vui lòng chọn kế hoạch khác.");
                            return;
                        }
                        var frmKH_XuatBaoDam_TuChoi = new frmKH_XuatBaoDam_PheDuyet(this.RefreshData, (int)MT.Data.iTrangThaiDuyetPNM.TuChoiDuyet, datasReject);
                        frmKH_XuatBaoDam_TuChoi.ShowDialog();
                        frmKH_XuatBaoDam_TuChoi.Dispose();
                    }
                    break;
            }
        }
        ...
        protected override void CustomLoadFinishDetailByMaster(MGridControl gridControl, IList data)
        {
            ...
            if (gridControl.TableName == "KH_XuatBaoDam_CT")
            {
        ...
        private void frmKH_XuatBaoDam_Load(object sender, EventArgs e)
        {
        ...
        private void backgroundWorkerPhuKien_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                e.Result = DBContext.GetRep<KH_XuatBaoDam_CTRepository>().GetDetailsByMasterID2("View_KH_XuatBaoDam_CT_PhuKien",
                        "KH_XuatBaoDam_CT_PhuKien", e.Argument);
            }
            finally
            {
