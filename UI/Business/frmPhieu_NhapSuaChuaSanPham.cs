...
namespace FormUI
{
    public partial class frmPhieu_NhapSuaChuaSanPham : FormUI.MFormList
    {
        #region"Property"
        ...
        {
            cboiLoaiNhapSuaChua.ItemEnum.Add(-1, "Tất cả");
            cboiLoaiNhapSuaChua.DefaultValueEnum = -1;
            drKyBaoCao.SetDefaultValue(MT.Library.Enummation.Period.NamNay);

            drKyBaoCao.Period_Changed = new EventHandler(LoadMasterData);

            cboiLoaiNhapSuaChua.EnumData = "iLoaiNhapSuaChua";
        ...
        private void InitGrid()
        {
            ...
            grd.ViewName = ((Phieu_NhapSuaChuaSanPhamRepository)ConfigFormDetail.Rep).GetViewPaging();
            ...
            //this.gridMaster.AddColumnText("sMaPhieu_NhapSuaChuaSanPham", "Mã phiếu xuất","Mã phiếu xuất", 180,isFixWidth:true,fixedStyle:DevExpress.XtraGrid.Columns.FixedStyle.Left);
            ...
            colMaster = this.gridMaster.AddColumnText("iLoai", "Tính chất nhập", 150,
              fixedStyle: DevExpress.XtraGrid.Columns.FixedStyle.Right);
            colMaster.EnumName = "iLoaiNhapSuaChua";
            colMaster.FilterEditor = new ConfigFilterEditor { DataType = DataTypeColumn.ComboBox };


            //Sản phẩm
            grdSanPham.TableName = "Phieu_NhapSuaChuaSanPham_CT";
            grdSanPham.ViewName = "View_Phieu_NhapSuaChuaSanPham_CT";
            ...
            grdPhuKienDetail.TableName = "Phieu_NhapSuaChuaSanPham_CT_PhuKien";
            grdPhuKienDetail.ViewName = "View_Phieu_NhapSuaChuaSanPham_CT_PhuKien";
        ...
        private bool CheckPermissionAproveOrReject(MT.Data.iTrangThaiDuyetPNSC iTrangThaiDuyet)
        {
            var datas = gridMaster.GetListRecordByRowsSelected<MT.Data.BO.Phieu_NhapSuaChuaSanPham>();
            ...
            if (iTrangThaiDuyet == MT.Data.iTrangThaiDuyetPNSC.TuChoiDuyet)
            {
                msg = "Từ chối";
            }
            ...
            foreach (var item in datas)
            {
                ...
                //List<string> pHXuatSP_sMas = DBContext.GetRep2<Phieu_NhapSuaChuaSanPhamRepository>().GetPhieuNhapsByPhX(item.Id);
        ...
        protected override void ProcessItemClick(int tag, object sender)
        {
            ...
            switch (tag)
            {
                ...
                case (int)MCommandName.Approve:
                    ...
                    if (CheckPermissionAproveOrReject(MT.Data.iTrangThaiDuyetPNSC.DongYDuyet))
                    {
                        var datasApprove = gridMaster.GetListRecordByRowsSelected<MT.Data.BO.Phieu_NhapSuaChuaSanPham>();
                        if (datasApprove.Any(m => m.iTrangThaiDuyet == (int)MT.Data.iTrangThaiDuyetPNSC.DongYDuyet))
                        {
                            string strMaApproves = string.Join(", ", datasApprove.FindAll(m => m.iTrangThaiDuyet == (int)MT.Data.iTrangThaiDuyetPNSC.DongYDuyet).Select(m => m.sMa));
                            MMessage.ShowWarning($"Các phiếu nhập <{strMaApproves}> đã được duyệt. Bạn vui lòng chọn phiếu khác.");
                            return;
                        }

                        var frmPhieu_NhapSuaChuaSanPham_PheDuyet = new frmPhieu_NhapSuaChuaSanPham_PheDuyet(this.RefreshData, (int)MT.Data.iTrangThaiDuyetPNSC.DongYDuyet, datasApprove);
                        frmPhieu_NhapSuaChuaSanPham_PheDuyet.ShowDialog();
                        frmPhieu_NhapSuaChuaSanPham_PheDuyet.Dispose();
                    }
                    break;

                case (int)MCommandName.Reject:
                    if (CheckPermissionAproveOrReject(MT.Data.iTrangThaiDuyetPNSC.TuChoiDuyet))
                    {
                        var datasReject = gridMaster.GetListRecordByRowsSelected<MT.Data.BO.Phieu_NhapSuaChuaSanPham>();
                        if (datasReject.Any(m => m.iTrangThaiDuyet == (int)MT.Data.iTrangThaiDuyetPNSC.TuChoiDuyet))
                        {
                            string strMaRejects = string.Join(", ", datasReject.FindAll(m => m.iTrangThaiDuyet == (int)MT.Data.iTrangThaiDuyetPNSC.TuChoiDuyet).Select(m => m.sMa));
                            MMessage.ShowWarning($"Các phiếu nhập <{strMaRejects}> đã bị từ chối. Bạn vui lòng chọn phiếu khác.");
                            return;
                        }
                        var frmPhieu_NhapSuaChuaSanPham_TuChoi = new frmPhieu_NhapSuaChuaSanPham_PheDuyet(this.RefreshData, (int)MT.Data.iTrangThaiDuyetPNSC.TuChoiDuyet, datasReject);
                        frmPhieu_NhapSuaChuaSanPham_TuChoi.ShowDialog();
                        frmPhieu_NhapSuaChuaSanPham_TuChoi.Dispose();
                    }
                    break;
            }
        }
        ...
        protected override void CustomLoadFinishDetailByMaster(MGridControl gridControl, IList data)
        {
            ...
            if (gridControl.TableName == "Phieu_NhapSuaChuaSanPham_CT")
            {
        ...
        private void LoadMasterData(object sender, EventArgs e)
        {
            try
            {
                ...
                PagingList.SetWhere($@" dNgayPhieu_Nhap BETWEEN '{drKyBaoCao.FromDate.Value.StartDate()}'
                                    ...
                                    AND (iLoai = {cboiLoaiNhapSuaChua.GetValue()} OR {cboiLoaiNhapSuaChua.GetValue()}<=-1)");
                RefreshData();
            }
            catch (Exception ex)
            {
        ...
        private void frmPhieu_NhapSuaChuaSanPham_Load(object sender, EventArgs e)
        {
        ...
        private void backgroundWorkerPhuKien_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                e.Result = DBContext.GetRep<Phieu_NhapSuaChuaSanPham_CTRepository>().GetDetailsByMasterID2("View_Phieu_NhapSuaChuaSanPham_CT_PhuKien",
                        "Phieu_NhapSuaChuaSanPham_CT_PhuKien", e.Argument);
            }
            finally
            {
        ...
        private void cboiLoaiNhapSuaChua_SelectedValueChanged(object sender, EventArgs e)
        {
