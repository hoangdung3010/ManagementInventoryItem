...
namespace FormUI
{
    public partial class frmPhieu_XuatSuaChuaSanPham : FormUI.MFormList
    {
        #region"Property"
        ...
        {
            cboiLoaiXuatSuaChua.ItemEnum.Add(-1, "Tất cả");
            cboiLoaiXuatSuaChua.DefaultValueEnum = -1;
            drKyBaoCao.SetDefaultValue(MT.Library.Enummation.Period.NamNay);

            drKyBaoCao.Period_Changed = new EventHandler(LoadMasterData);

            cboiLoaiXuatSuaChua.EnumData = "iLoaiXuatSuaChua";
        ...
        private void InitGrid()
        {
            ...
            grd.ViewName = ((Phieu_XuatSuaChuaSanPhamRepository)ConfigFormDetail.Rep).GetViewPaging();
            ...
            var colMaster = this.gridMaster.AddColumnText("sMa", "Số phiếu", "Số phiếu", 80, isFixWidth: true,
                fixedStyle: DevExpress.XtraGrid.Columns.FixedStyle.Left);
            this.gridMaster.AddColumnText("sMa_KH", "Căn cứ số", "Căn cứ số", 80, isFixWidth: true);
            this.gridMaster.AddColumnText("dNgayPhieu_Xuat", "Ngày lập phiếu", 80,
                dataType: DataTypeColumn.DateEdit);
            //this.gridMaster.AddColumnText("sMaPhieu_XuatSuaChuaSanPham", "Mã phiếu xuất","Mã phiếu xuất", 180,isFixWidth:true,fixedStyle:DevExpress.XtraGrid.Columns.FixedStyle.Left);
            //this.gridMaster.AddColumnText("sKH_XuatSuaChuaSanPham_Id_CanCu", "Kế hoạch xuất", 150,fixedStyle: DevExpress.XtraGrid.Columns.FixedStyle.Left);
            ...
            colMaster = this.gridMaster.AddColumnText("iLoai", "Tính chất xuất", 150,
              fixedStyle: DevExpress.XtraGrid.Columns.FixedStyle.Right);
            colMaster.EnumName = "iLoaiXuatSuaChua";
            colMaster.FilterEditor = new ConfigFilterEditor { DataType = DataTypeColumn.ComboBox };

            //Sản phẩm
            grdSanPham.TableName = "Phieu_XuatSuaChuaSanPham_CT";
            grdSanPham.ViewName = "View_Phieu_XuatSuaChuaSanPham_CT";
            ...
            col = grdSanPham.AddColumnText("sDM_NoiDungSuaChua_Id_Ten", "Nội dung sửa chữa", 150);
            ...
            grdPhuKienDetail.TableName = "Phieu_XuatSuaChuaSanPham_CT_PhuKien";
            grdPhuKienDetail.ViewName = "View_Phieu_XuatSuaChuaSanPham_CT_PhuKien";
        ...
        private bool CheckPermissionAproveOrReject(MT.Data.iTrangThaiDuyetPXSC iTrangThaiDuyetKHNM)
        {
            var datas = gridMaster.GetListRecordByRowsSelected<MT.Data.BO.Phieu_XuatSuaChuaSanPham>();
            ...
            if (iTrangThaiDuyetKHNM == MT.Data.iTrangThaiDuyetPXSC.TuChoiDuyet)
            {
                msg = "Từ chối";
            }
            ...
            foreach (var item in datas)
            {
                ...
                List<string> pHXuatSP_sMas = DBContext.GetRep2<Phieu_XuatSuaChuaSanPhamRepository>().GetPhieuNhapsByPhX(item.Id);
        ...
        protected override void ProcessItemClick(int tag, object sender)
        {
            ...
            switch (tag)
            {
                ...
                case (int)MCommandName.Approve:
                    ...
                    if (CheckPermissionAproveOrReject(MT.Data.iTrangThaiDuyetPXSC.DongYDuyet))
                    {
                        var datasApprove = gridMaster.GetListRecordByRowsSelected<MT.Data.BO.Phieu_XuatSuaChuaSanPham>();
                        if (datasApprove.Any(m => m.iTrangThaiDuyet == (int)MT.Data.iTrangThaiDuyetPXSC.DongYDuyet))
                        {
                            string strMaApproves = string.Join(", ", datasApprove.FindAll(m => m.iTrangThaiDuyet == (int)MT.Data.iTrangThaiDuyetPXSC.DongYDuyet).Select(m => m.sMa));
                            MMessage.ShowWarning($"Các phiếu xuất <{strMaApproves}> đã được duyệt. Bạn vui lòng chọn phiếu khác.");
                            return;
                        }

                        var frmPhieu_XuatSuaChuaSanPham_PheDuyet = new frmPhieu_XuatSuaChuaSanPham_PheDuyet(this.RefreshData, (int)MT.Data.iTrangThaiDuyetPXSC.DongYDuyet, datasApprove);
                        frmPhieu_XuatSuaChuaSanPham_PheDuyet.ShowDialog();
                        frmPhieu_XuatSuaChuaSanPham_PheDuyet.Dispose();
                    }
                    break;

                case (int)MCommandName.Reject:
                    if (CheckPermissionAproveOrReject(MT.Data.iTrangThaiDuyetPXSC.TuChoiDuyet))
                    {
                        var datasReject = gridMaster.GetListRecordByRowsSelected<MT.Data.BO.Phieu_XuatSuaChuaSanPham>();
                        if (datasReject.Any(m => m.iTrangThaiDuyet == (int)MT.Data.iTrangThaiDuyetPXSC.TuChoiDuyet))
                        {
                            string strMaRejects = string.Join(", ", datasReject.FindAll(m => m.iTrangThaiDuyet == (int)MT.Data.iTrangThaiDuyetPXSC.TuChoiDuyet).Select(m => m.sMa));
                            MMessage.ShowWarning($"Các phiếu xuất <{strMaRejects}> đã bị từ chối. Bạn vui lòng chọn phiếu khác.");
                            return;
                        }
                        var frmPhieu_XuatSuaChuaSanPham_TuChoi = new frmPhieu_XuatSuaChuaSanPham_PheDuyet(this.RefreshData, (int)MT.Data.iTrangThaiDuyetPXSC.TuChoiDuyet, datasReject);
                        frmPhieu_XuatSuaChuaSanPham_TuChoi.ShowDialog();
                        frmPhieu_XuatSuaChuaSanPham_TuChoi.Dispose();
                    }
                    break;
            }
        }
        ...
        protected override void CustomLoadFinishDetailByMaster(MGridControl gridControl, IList data)
        {
            ...
            if (gridControl.TableName == "Phieu_XuatSuaChuaSanPham_CT")
            {
        ...
        private void LoadMasterData(object sender, EventArgs e)
        {
            try
            {
                ...
                PagingList.SetWhere($@" dNgayPhieu_Xuat BETWEEN '{drKyBaoCao.FromDate.Value.StartDate()}'
                                    ...
                                    AND (iLoai={cboiLoaiXuatSuaChua.GetValue()} OR {cboiLoaiXuatSuaChua.GetValue()}<=-1)");
                RefreshData();
            }
            catch (Exception ex)
            {
        ...
        private void frmPhieu_XuatSuaChuaSanPham_Load(object sender, EventArgs e)
        {
        ...
        private void backgroundWorkerPhuKien_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                e.Result = DBContext.GetRep<Phieu_XuatSuaChuaSanPham_CTRepository>().GetDetailsByMasterID2("View_Phieu_XuatSuaChuaSanPham_CT_PhuKien",
                        "Phieu_XuatSuaChuaSanPham_CT_PhuKien", e.Argument);
            }
            finally
            {
        ...
        private void cboiLoaiXuatSuaChua_SelectedValueChanged(object sender, EventArgs e)
        {
