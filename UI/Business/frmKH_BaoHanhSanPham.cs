...
namespace FormUI
{
    public partial class frmKH_BaoHanhSanPham : FormUI.MFormList
    {
        #region"Property"
        ...
        private void InitGrid()
        {
            ...
            //Sản phẩm
            grdSanPham.TableName = "KH_BaoHanhSanPham_CT";
            grdSanPham.ViewName = "View_KH_BaoHanhSanPham_CT";
            ...
            grdPhuKienDetail.TableName = "KH_BaoHanhSanPham_CT_PhuKien";
            grdPhuKienDetail.TableName = "View_KH_BaoHanhSanPham_CT_PhuKien";
        ...
        protected override void ProcessItemClick(int tag, object sender)
        {
            switch (tag)
            {
                case (int)MCommandName.Edit:
                    var selectedItem = gridMaster.GetRecordByRowSelected<MT.Data.BO.KH_BaoHanhSanPham>();
                    // Nếu kế hoạch sửa chữa đã phát sinh phiếu xuất sửa chữa mới thì không cho sửa
                    List<string> phieuXuatBaoHanh_sMas = DBContext.GetRep2<KH_BaoHanhSanPhamRepository>().GetPhieuXuatBaoHanhsByKH(selectedItem.Id);
                    if (phieuXuatBaoHanh_sMas.Count > 0)
                    {
                        MMessage.ShowWarning($"Bạn không được phép sửa. Do kế hoạch sửa chữa <{selectedItem.sMa}> đã phát sinh các phiếu xuất sửa chữa mới <{string.Join(", ", phieuXuatBaoHanh_sMas)}>.");
                        return;
                    }
                    break;
            }
            ...
            switch (tag)
            {
                case (int)MCommandName.Approve:
                    ...
                    {
                        var datasApprove = gridMaster.GetListRecordByRowsSelected<MT.Data.BO.KH_BaoHanhSanPham>();
                        ...
                        var frmKH_BaoHanhSanPham_PheDuyet = new frmKH_BaoHanhSanPham_PheDuyet(this.RefreshData, (int)MT.Data.iTrangThaiDuyetKHNM.DongYDuyet, datasApprove);
                        frmKH_BaoHanhSanPham_PheDuyet.ShowDialog();
                        frmKH_BaoHanhSanPham_PheDuyet.Dispose();
                    }
                    break;
                ...
                case (int)MCommandName.Reject:
                    ...
                    {
                        var datasReject = gridMaster.GetListRecordByRowsSelected<MT.Data.BO.KH_BaoHanhSanPham>();
                        ...
                        var frmKH_BaoHanhSanPham_TuChoi = new frmKH_BaoHanhSanPham_PheDuyet(this.RefreshData, (int)MT.Data.iTrangThaiDuyetKHNM.TuChoiDuyet, datasReject);
                        frmKH_BaoHanhSanPham_TuChoi.ShowDialog();
                        frmKH_BaoHanhSanPham_TuChoi.Dispose();
                    }
                    break;
            }
        }
        ...
        protected override void CustomLoadFinishDetailByMaster(MGridControl gridControl, IList data)
        {
            ...
            if (gridControl.TableName == "KH_BaoHanhSanPham_CT")
            {
        ...
        private void frmKH_BaoHanhSanPham_Load(object sender, EventArgs e)
        {
        ...
        private void backgroundWorkerPhuKien_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                e.Result = DBContext.GetRep<KH_BaoHanhSanPham_CTRepository>().GetDetailsByMasterID2("View_KH_BaoHanhSanPham_CT_PhuKien",
                        "KH_BaoHanhSanPham_CT_PhuKien", e.Argument);
            }
            finally
            {
        ...
        {
            var datas = gridMaster.GetListRecordByRowsSelected<MT.Data.BO.KH_BaoHanhSanPham>();
            ...
            foreach (var item in datas)
            {
                ...
                List<string> phieuXuatBaoHanh_sMas = DBContext.GetRep2<KH_BaoHanhSanPhamRepository>().GetPhieuXuatBaoHanhsByKH(item.Id);
                if (phieuXuatBaoHanh_sMas.Count > 0)
                {
                    MMessage.ShowWarning($"Bạn không được phép {msg}. Do kế hoạch sửa chữa <{item.sMa}> đã phát sinh các phiếu xuất sửa chữa <{string.Join(", ", phieuXuatBaoHanh_sMas)}>.");
                    return false;
                }
            }
