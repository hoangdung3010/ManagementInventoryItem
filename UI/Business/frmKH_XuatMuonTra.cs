...
namespace FormUI
{
    public partial class frmKH_XuatMuonTra : FormUI.MFormList
    {
        #region"Property"
        ...
        private void InitGrid()
        {
            ...
            grd.ViewName = ((KH_XuatMuonTraRepository)ConfigFormDetail.Rep).GetViewPaging();
            ...
            colMaster.FilterEditor = new ConfigFilterEditor { DataType = DataTypeColumn.ComboBox };
            //Sản phẩm
            grdSanPham.TableName = "KH_XuatMuonTra_CT";
            grdSanPham.ViewName = "View_KH_XuatMuonTra_CT";
            ...
            col = grdSanPham.AddColumnText("sDM_HangMucMuonTra_Id_Ten", "Loại hạng mục", 180, isFixWidth: true);
            ...
            grdPhuKienDetail.TableName = "KH_XuatMuonTra_CT_PhuKien";
            grdPhuKienDetail.ViewName = "View_KH_XuatMuonTra_CT_PhuKien";
        ...
        {
            var datas = gridMaster.GetListRecordByRowsSelected<MT.Data.BO.KH_XuatMuonTra>();

            if (datas == null || datas.Count == 0)
            {
                MMessage.ShowWarning("Bạn phải chọn ít nhất 1 kế hoạch Xuất cho mượn.");
                return false;
            }
            ...
            foreach (var item in datas)
            {
                ...
                {
                    MMessage.ShowWarning($"Bạn không có quyền {msg} với kế hoạch Xuất cho mượn <{item.sMa}>.");
                    return false;
                }

                //Nếu Kế hoạch đã phát sinh phiếu nhập thì không cho phế duyệt hoặc từ chối
                //Nếu kế hoạch Xuất cho mượn đã phát sinh kế hoạch xuất thì không cho sửa
                ...
                {
                    MMessage.ShowWarning($"Bạn không được phép {msg}. Do kế hoạch Xuất cho mượn <{item.sMa}> đã phát sinh các kế hoạch xuất <{string.Join(", ", kHXuatSP_sMas)}>.");
                    return false;
                }
            }
        ...
        protected override void ProcessItemClick(int tag, object sender)
        {
            ...
            switch (tag)
            {
                ...
                //    var data = gridMaster.GetRecordByRowSelected<MT.Data.BO.KH_XuatMuonTra>();
                //    frmKH_XuatMuonTra_PheDuyet frmKH_XuatMuonTra_PheDuyet = new frmKH_XuatMuonTra_PheDuyet(data);
                //    frmKH_XuatMuonTra_PheDuyet.ShowDialog();
                //    frmKH_XuatMuonTra_PheDuyet.Dispose();
                //    break;
                ...
                case (int)MCommandName.Approve:
                    ...
                    {
                        var datasApprove = gridMaster.GetListRecordByRowsSelected<MT.Data.BO.KH_XuatMuonTra>();
                        if (datasApprove.Any(m => m.iTrangThaiDuyet == (int)MT.Data.iTrangThaiDuyetPNM.DongYDuyet))
                        {
                            ...
                            MMessage.ShowWarning($"Các kế hoạch Xuất cho mượn <{strMaApproves}> đã được duyệt. Bạn vui lòng chọn kế hoạch khác.");
                            return;
                        }

                        var frmKH_XuatMuonTra_PheDuyet = new frmKH_XuatMuonTra_PheDuyet(this.RefreshData, (int)MT.Data.iTrangThaiDuyetPNM.DongYDuyet, datasApprove);
                        frmKH_XuatMuonTra_PheDuyet.ShowDialog();
                        frmKH_XuatMuonTra_PheDuyet.Dispose();
                    }
                    break;
                ...
                case (int)MCommandName.Reject:
                    ...
                    {
                        var datasReject = gridMaster.GetListRecordByRowsSelected<MT.Data.BO.KH_XuatMuonTra>();
                        if (datasReject.Any(m => m.iTrangThaiDuyet == (int)MT.Data.iTrangThaiDuyetPNM.TuChoiDuyet))
                        {
                            ...
                            MMessage.ShowWarning($"Các kế hoạch Xuất cho mượn <{strMaRejects}> đã bị từ chối. Bạn vui lòng chọn kế hoạch khác.");
                            return;
                        }
                        var frmKH_XuatMuonTra_TuChoi = new frmKH_XuatMuonTra_PheDuyet(this.RefreshData, (int)MT.Data.iTrangThaiDuyetPNM.TuChoiDuyet, datasReject);
                        frmKH_XuatMuonTra_TuChoi.ShowDialog();
                        frmKH_XuatMuonTra_TuChoi.Dispose();
                    }
                    break;
            }
        }
        ...
        protected override void CustomLoadFinishDetailByMaster(MGridControl gridControl, IList data)
        {
            ...
            if (gridControl.TableName == "KH_XuatMuonTra_CT")
            {
        ...
        private void frmKH_XuatMuonTra_Load(object sender, EventArgs e)
        {
        ...
        private void backgroundWorkerPhuKien_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                e.Result = DBContext.GetRep<KH_XuatMuonTra_CTRepository>().GetDetailsByMasterID2("View_KH_XuatMuonTra_CT_PhuKien",
                        "KH_XuatMuonTra_CT_PhuKien", e.Argument);
            }
            finally
            {
