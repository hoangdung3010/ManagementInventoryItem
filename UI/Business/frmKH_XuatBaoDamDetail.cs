...
namespace FormUI
{
    public partial class frmKH_XuatBaoDamDetail : FormUI.MFormBusinessDetail
    {
        private DM_DonViRepository dM_DonViRepository;

        public frmKH_XuatBaoDamDetail()
        {
            InitializeComponent();
            ...
            if (ConfigFormDetail == null)
            {
                ...
                {
                    Rep = DBContext.GetRep<KH_XuatBaoDamRepository>(),
                    EntityName = "KH_XuatBaoDam",
                    Title = "Kế hoạch bảo đảm"
                };
            }

            GrdDetails = new Dictionary<string, MTControl.MGridEditable>
            {
                {"KH_XuatBaoDam_CT",grdSanPham }
            };
        ...
        private void InitLookup()
        {
            ...
            //cboTrangThaiDuyet.EnumData = "iTrangThaiDuyetKHNM";
        }
        ...
        private void InitGrid()
        {
            ...
            //var dm_DonViTinhs = DBContext.GetRep<MT.Data.Rep.DM_DonViTinhRepository>().GetData(typeof(MT.Data.BO.DM_DonViTinh), "Id,sTenDonViTinh", orderBy: "SortOrder");
            //Sản phẩm
            grdSanPham.GrdDetail.TableName = "KH_XuatBaoDam_CT";
            grdSanPham.GrdDetail.ViewName = "View_KH_XuatBaoDam_CT";
            grdSanPham.GrdDetail.KeyName = "Id";
            grdSanPham.GrdDetail.SetField = "kH_XuatBaoDam_CTs";
            ...
            // Số hợp đồng
            // Cách 1:
            //col= grdSanPham.GrdDetail.AddColumnText("sSoHopDong", "Số HĐ", 150, maxLength: 255);
            // Cách 2: --> ok
            // fixedStyle: DevExpress.XtraGrid.Columns.FixedStyle.Left: DO anh dang neo cột , cố định cột thì nó bị vậy, nếu đã neo thì  ahieu tất ok e a hieu roi
            col = grdSanPham.GrdDetail.AddColumnText("sSoHopDong", "Số HĐ", 250, isFixWidth: true,
               fixedStyle: DevExpress.XtraGrid.Columns.FixedStyle.Left,
               dataType: MTControl.DataTypeColumn.None, isRequired: true);

            // Đơn vị nhập
            col = grdSanPham.GrdDetail.AddColumnText("sDM_DonVi_Id_DonViNhap", "Đơn vị nhập", 180, isFixWidth: true,
               fixedStyle: DevExpress.XtraGrid.Columns.FixedStyle.Left,
               dataType: MTControl.DataTypeColumn.TreeLookUpEdit, isRequired: true);
            MRepositoryItemTreeLookUpEdit colTreesDM_DonVi_Id_DonViNhap = (MRepositoryItemTreeLookUpEdit)col.ColumnEdit;
            colTreesDM_DonVi_Id_DonViNhap.DictionaryName = "DM_DonVi";
            MTreeLookUpEdit treeLookup = new MTreeLookUpEdit();
            treeLookup.Properties.DisplayMember = "sTenDonvi";
            treeLookup.Properties.ValueMember = "Id";
            var treeList = treeLookup.Properties.TreeList;
            treeList.KeyFieldName = "Id";
            treeList.ParentFieldName = "sParentId";
            treeLookup.AliasFormQuickAdd = "DM_DonVi";
            treeLookup.AddColumn("sMaDonvi", "Mã đơn vị", 120);
            treeLookup.AddColumn("sTenDonvi", "Tên đơn vị", 180, true);
            colTreesDM_DonVi_Id_DonViNhap.TreeList = treeList;
            colTreesDM_DonVi_Id_DonViNhap.DataSource = dM_DonViRepository.GetDonViConCap1VaDonViCungCap(MT.Library.SessionData.OrganizationUnitId);
            colTreesDM_DonVi_Id_DonViNhap.DisplayMember = "sTenDonvi";
            colTreesDM_DonVi_Id_DonViNhap.ValueMember = "Id";

            // Loại hạng mục
            col = grdSanPham.GrdDetail.AddColumnText("sDM_HangMucBaoDam_Id", "Loại hạng mục bảo đảm", 150, isFixWidth: true,
               //fixedStyle: DevExpress.XtraGrid.Columns.FixedStyle.Left,
               dataType: MTControl.DataTypeColumn.LookUpEdit, isRequired: false);
            MRepositoryItemLookUpEdit colsDM_HangMucLL_Id = (MRepositoryItemLookUpEdit)col.ColumnEdit;
            colsDM_HangMucLL_Id.DictionaryName = "DM_HangMucBaoDam";
            //colsDM_MangLL_Id.AddColumn("sMaMangLienLac", "Mã mạng liên lạc", 120);
            colsDM_HangMucLL_Id.AddColumn("sTenHangMuc", "Tên hạng mục liên lạc", 180);
            colsDM_HangMucLL_Id.DataSource = DBContext.GetRep<MT.Data.Rep.DM_HangMucBaoDamRepository>().GetData(typeof(MT.Data.BO.DM_HangMucBaoDam), "Id,sTenHangMuc", orderBy: "SortOrder"); ;
            colsDM_HangMucLL_Id.DisplayMember = "sTenHangMuc";
            colsDM_HangMucLL_Id.ValueMember = "Id";
            ...
            col = grdSanPham.GrdDetail.AddColumnText("sDM_SanPham_Id", "Hành động", 100, dataType: MTControl.DataTypeColumn.Button,
        ...
        private bool grdSanPham_ValidEditValueChanging(DevExpress.XtraGrid.Columns.GridColumn gridColumn, ChangingEventArgs e)
        {
            if (gridColumn.FieldName == "sSerial" && e.NewValue != null && !object.Equals(e.OldValue, e.NewValue))
            {
                ...
                KH_XuatBaoDam_CT kH_XuatBaoDam = grdSanPham.GrdDetail.GetRecordByRowSelected<KH_XuatBaoDam_CT>();
                if (arrSerial.Length > 0 && (decimal)arrSerial.Length > kH_XuatBaoDam.rSoLuong)
                {
                    e.NewValue = string.Empty;
                    MMessage.ShowWarning($"Bạn chỉ được phép chọn tối đa {(int)kH_XuatBaoDam.rSoLuong} số serial");
                    return false;
                }
            }
        ...
        private void ColsActionPhuKien_ButtonClick(object sender, ButtonPressedEventArgs e)
        {
            ...
            var kH_XuatBaoDam_CT = grdSanPham.GrdDetail.GetRecordByRowSelected<KH_XuatBaoDam_CT>();
            if (kH_XuatBaoDam_CT.sDM_SanPham_Id.HasValue && !kH_XuatBaoDam_CT.sDM_SanPham_Id.Value.Equals(Guid.Empty))
            {
                frmPhuKienDetail frmPhuKienDetail = new frmPhuKienDetail(kH_XuatBaoDam_CT.sDM_SanPham_Id.Value,
                     "KH_XuatBaoDam_CT_PhuKien", "kH_XuatBaoDam_CT_PhuKiens", kH_XuatBaoDam_CT, this);
        ...
        protected override void SetConfigBeforePrint(MT.Data.BO.ConfigExcel configExcel, ConfigReport configReport)
        {
            ...
            configReport.RepName = "Print_KH_XuatBaoDam_DetailRepository";
            //Định danh mẫu in trong bảng ReportData
            configReport.DictionaryKey = MT.Data.ReportDictionaryKey.RP_Print_KH_XuatBaoDamDetail;

            //Danh sách các cột trên table
            configExcel.ShowColumnsOrder = new HashSet<string>
            {
                "sSTT","sDM_SanPham_Id_Ten","sSoHopDong","sDM_DonVi_Id_DonViNhap_Ten","sDM_HangMucBaoDam_Id_Ten","sDM_DonViTinh_Id_Ten","rSoLuong","sXuatXu"
            };
        }
        ...
        protected override bool IsValidateDataChangedGridDetail(MGridEditable mGridEditable, System.Collections.IList dataChanged)
        {
            ...
            {
                List<MT.Data.BO.KH_XuatBaoDam_CT> kH_XuatBaoDam_CTs = mGridEditable.GrdDetail.GetAllData<MT.Data.BO.KH_XuatBaoDam_CT>();

                foreach (var item in dataChanged)
                {
                    var castItem = (MT.Data.BO.KH_XuatBaoDam_CT)item;
        ...
        protected override void CustomRowCellGridDetail(MGridControl mGridControl, RepositoryItem repository, CustomRowCellEditEventArgs e)
        {
            ...
            switch (mGridControl.TableName)
            {
                case "KH_XuatBaoDam_CT":
                    ...
                    {
                        //MT.Data.BO.KH_XuatBaoDam curMaster = (MT.Data.BO.KH_XuatBaoDam)this.CurrentData;
        ...
        protected override void CustomRowCellValueChangedGridDetail(MGridControl mGridControl, CellValueChangedEventArgs e)
        {
            ...
            {
                case "KH_XuatBaoDam_CT":
                    KH_XuatBaoDam_CT kH_XuatBaoDam_CT = mGridControl.GetRecordByRowSelected<KH_XuatBaoDam_CT>();
                    if (e.Column.FieldName == "sDM_SanPham_Id")
                    {
                        //gán mặc định đơn vị tính cấp 1 cho sản phẩm
                        DM_SanPhamRepository dM_SanPhamRepository = DBContext.GetRep2<DM_SanPhamRepository>();
                        ...
                        if (dM_SanPham != null)
                        {
                            kH_XuatBaoDam_CT.sDM_DonViTinh_Id = dM_SanPham.sDM_DonViTinh_Id_Cap1;
                            kH_XuatBaoDam_CT.rSoLuong = 1;
                            kH_XuatBaoDam_CT.rDonGia = dM_SanPham.rDonGia_Cap1;
                            kH_XuatBaoDam_CT.rThanhTien = dM_SanPham.rDonGia_Cap1;
                            ...
                            if (!kH_XuatBaoDam_CT.IsLoaded && kH_XuatBaoDam_CT.MTEntityState == Enummation.MTEntityState.Add)
                            {
                                var phukiensCuaSanPham = (List<DM_PhuKien>)DBContext.GetRep2<DM_PhuKienRepository>()
                                                                ...
                                                                , where: $"sDM_SanPham_Id='{kH_XuatBaoDam_CT.sDM_SanPham_Id.Value}'"
                                                                , orderBy: "SortOrder");
                                List<KH_XuatBaoDam_CT_PhuKien> phukienCuaKeHoachNhapMoi = new List<KH_XuatBaoDam_CT_PhuKien>();
                                if (phukiensCuaSanPham != null)
                                {
                                    foreach (var item in phukiensCuaSanPham)
                                    {
                                        phukienCuaKeHoachNhapMoi.Add(new KH_XuatBaoDam_CT_PhuKien
                                        {
                                            Id = Guid.NewGuid(),
                                            rSoLuongPhuKienTren1SP = item.rSoLuong,
                                            rSoLuong = kH_XuatBaoDam_CT.rSoLuong * item.rSoLuong,
                                            ...
                                            rThanhTien = kH_XuatBaoDam_CT.rSoLuong * item.rSoLuong * item.rDonGia,
                                            sDM_SanPham_Id = item.sDM_SanPham_Id
                                        });
                                    }
                                }
                                kH_XuatBaoDam_CT.kH_XuatBaoDam_CT_PhuKiens = phukienCuaKeHoachNhapMoi;
                            }
                        ...
                        {
                            mGridControl.FuncCustomRecordBeforeAddRow(kH_XuatBaoDam_CT, mGridControl);
                        }
                    }
                    if (e.Column.FieldName == "sDM_DonViTinh_Id")
                    {
                    ...
                    {
                        kH_XuatBaoDam_CT.rThanhTien = kH_XuatBaoDam_CT.rSoLuong * kH_XuatBaoDam_CT.rDonGia;
                    }
                    if (e.Column.FieldName == "sSerial")
                    {
                        ...
                        if (objsSerial != null)
                        {
                            ...
                            {
                                kH_XuatBaoDam_CT.rSoLuong = arrSerial.Length;
                            }
                        }
                    }
                    ...
                    if (e.Column.FieldName == "iThoiGianBaoHanh" || e.Column.FieldName == "iDonViThoiGianBaoHanh")
                    {
                        ...
                        switch (kH_XuatBaoDam_CT.iDonViThoiGianBaoHanh)
                        {
                            case (int)MT.Data.iDonViThoiGianHieuLuc.Ngay:
                                dHanBaoHanhDen = now.AddDays(kH_XuatBaoDam_CT.iThoiGianBaoHanh).AddDays(1);
                                break;

                            case (int)MT.Data.iDonViThoiGianHieuLuc.Thang:
                                dHanBaoHanhDen = now.AddMonths(kH_XuatBaoDam_CT.iThoiGianBaoHanh).AddDays(1);
                                break;

                            case (int)MT.Data.iDonViThoiGianHieuLuc.Quy:
                                dHanBaoHanhDen =now.AddYears(kH_XuatBaoDam_CT.iThoiGianBaoHanh * 3).AddDays(1);
                                break;

                            case (int)MT.Data.iDonViThoiGianHieuLuc.Nam:
                                dHanBaoHanhDen = now.AddYears(kH_XuatBaoDam_CT.iThoiGianBaoHanh).AddDays(1);
                                break;
                        }
                        kH_XuatBaoDam_CT.dHanBaoHanhDen = dHanBaoHanhDen;
                    }
                    ...
                    //if (e.Column.FieldName == "rSoLuong" || e.Column.FieldName == "rDonGia")
                    //{
                    //    var kH_XuatBaoDam_CT = mGridControl.GetRecordByRowSelected<KH_XuatBaoDam_CT>();
                    //    mGridControl.FirstView.SetRowCellValue(e.RowHandle, "rThanhTien", kH_XuatBaoDam_CT.rSoLuong * kH_XuatBaoDam_CT.rDonGia);
                    //}
        ...
        protected override void BindingDataIntoFormSucces()
        {
            ...
            switch (this.FormAction)
            {
                ...
                case (int)MTControl.FormAction.View:
                    ...
                    ucThamChieuPhieuNhapMoi.sTenBangCanCu = "KH_XuatBaoDam";
        ...
        {
            var currentData = (KH_XuatBaoDam)base.PrepareDataBeforeBindDataForm();
