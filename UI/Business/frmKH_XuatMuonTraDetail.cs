...
namespace FormUI
{
    public partial class frmKH_XuatMuonTraDetail : FormUI.MFormBusinessDetail
    {
        private DM_DonViRepository dM_DonViRepository;

        public frmKH_XuatMuonTraDetail()
        {
            InitializeComponent();
            ...
            if (ConfigFormDetail == null)
            {
                ...
                {
                    Rep = DBContext.GetRep<KH_XuatMuonTraRepository>(),
                    EntityName = "KH_XuatMuonTra",
                    Title = "Kế hoạch xuất cho mượn"
                };
            }

            GrdDetails = new Dictionary<string, MTControl.MGridEditable>
            {
                {"KH_XuatMuonTra_CT",grdSanPham }
            };
        ...
        private void InitGrid()
        {
            ...
            //var dm_DonViTinhs = DBContext.GetRep<MT.Data.Rep.DM_DonViTinhRepository>().GetData(typeof(MT.Data.BO.DM_DonViTinh), "Id,sTenDonViTinh", orderBy: "SortOrder");
            //Sản phẩm
            grdSanPham.GrdDetail.TableName = "KH_XuatMuonTra_CT";
            grdSanPham.GrdDetail.ViewName = "View_KH_XuatMuonTra_CT";
            grdSanPham.GrdDetail.KeyName = "Id";
            grdSanPham.GrdDetail.SetField = "kH_XuatMuonTra_CTs";
        ...
        private bool grdSanPham_ValidEditValueChanging(DevExpress.XtraGrid.Columns.GridColumn gridColumn, ChangingEventArgs e)
        {
            if (gridColumn.FieldName == "sSerial" && e.NewValue != null && !object.Equals(e.OldValue, e.NewValue))
            {
                ...
                KH_XuatMuonTra_CT kH_XuatMuonTra = grdSanPham.GrdDetail.GetRecordByRowSelected<KH_XuatMuonTra_CT>();
                if (arrSerial.Length > 0 && (decimal)arrSerial.Length > kH_XuatMuonTra.rSoLuong)
                {
                    e.NewValue = string.Empty;
                    MMessage.ShowWarning($"Bạn chỉ được phép chọn tối đa {(int)kH_XuatMuonTra.rSoLuong} số serial");
                    return false;
                }
            }
        ...
        private void ColsActionPhuKien_ButtonClick(object sender, ButtonPressedEventArgs e)
        {
            ...
            var kH_XuatMuonTra_CT = grdSanPham.GrdDetail.GetRecordByRowSelected<KH_XuatMuonTra_CT>();
            if (kH_XuatMuonTra_CT.sDM_SanPham_Id.HasValue && !kH_XuatMuonTra_CT.sDM_SanPham_Id.Value.Equals(Guid.Empty))
            {
                frmPhuKienDetail frmPhuKienDetail = new frmPhuKienDetail(kH_XuatMuonTra_CT.sDM_SanPham_Id.Value,
                     "KH_XuatMuonTra_CT_PhuKien", "kH_XuatMuonTra_CT_PhuKiens", kH_XuatMuonTra_CT, this);
        ...
        protected override void SetConfigBeforePrint(MT.Data.BO.ConfigExcel configExcel, ConfigReport configReport)
        {
            ...
            configReport.RepName = "Print_KH_XuatMuonTra_DetailRepository";
            //Định danh mẫu in trong bảng ReportData
            configReport.DictionaryKey = MT.Data.ReportDictionaryKey.RP_Print_KH_XuatMuonTraDetail;
        ...
        protected override bool IsValidateDataChangedGridDetail(MGridEditable mGridEditable, System.Collections.IList dataChanged)
        {
            ...
            {
                List<MT.Data.BO.KH_XuatMuonTra_CT> kH_XuatMuonTra_CTs = mGridEditable.GrdDetail.GetAllData<MT.Data.BO.KH_XuatMuonTra_CT>();

                foreach (var item in dataChanged)
                {
                    var castItem = (MT.Data.BO.KH_XuatMuonTra_CT)item;
                    // Truong 2022
                    KH_XuatMuonTra_CT findBysID = kH_XuatMuonTra_CTs.Find(m => m.sDM_SanPham_Id.Equals(castItem.sDM_SanPham_Id) && m.Id != castItem.Id);
                    if (findBysID != null)
                    {
        ...
        protected override void CustomRowCellGridDetail(MGridControl mGridControl, RepositoryItem repository, CustomRowCellEditEventArgs e)
        {
            ...
            switch (mGridControl.TableName)
            {
                case "KH_XuatMuonTra_CT":
                    ...
                    //{
                    //    //gán mặc định đơn vị tính cấp 1 cho sản phẩm
                    //    KH_XuatMuonTra_CTRepository kh_XuatSanPham_CTRepository = DBContext.GetRep2<KH_XuatMuonTra_CTRepository>();
                    //    object sKH_XuatMuonTra_Id_CanCu = cbossKH_XuatMuonTra_Id_CanCu.EditValue;
                    //    MRepositoryItemLookUpEdit repositoryItemLookUpEdit = (MRepositoryItemLookUpEdit)repository;
                    //    if (sKH_XuatMuonTra_Id_CanCu != null)
                    //    {
                    //        repositoryItemLookUpEdit.DataSource = kh_XuatSanPham_CTRepository
                    //                                                .GetKH_XuaMuonTra_CTsBysKH_XuatMuonTra_Id(Guid.Parse(sKH_XuatMuonTra_Id_CanCu.ToString()));
                    //    }
                    //    else
                    //    {
                    //        repositoryItemLookUpEdit.DataSource = null;
                    ...
                    {
                        //MT.Data.BO.KH_XuatMuonTra curMaster = (MT.Data.BO.KH_XuatMuonTra)this.CurrentData;
                        object sDM_Donvi_Id = treesDM_DonVi_Id_DonViXuat.GetValue();// this.CurrentData.GetValue("sDM_DonVi_Id_DonViXuat");
        ...
        protected override void CustomRowCellValueChangedGridDetail(MGridControl mGridControl, CellValueChangedEventArgs e)
        {
            ...
            switch (mGridControl.TableName)
            {
                case "KH_XuatMuonTra_CT":
                    ...
                    KH_XuatMuonTra_CT kH_XuatMuonTra_CT = mGridControl.GetRecordByRowSelected<KH_XuatMuonTra_CT>();
                    if (e.Column.FieldName == "sDM_SanPham_Id")
                    {
                        // Truong2022
                        List<MT.Data.BO.KH_XuatMuonTra_CT> kH_XuatMuonTra_CTs = grdSanPham.GrdDetail.GetAllData<MT.Data.BO.KH_XuatMuonTra_CT>();
                        KH_XuatMuonTra_CT findBysID = kH_XuatMuonTra_CTs.Find(m => m.sDM_SanPham_Id.Equals(kH_XuatMuonTra_CT.sDM_SanPham_Id) && m.Id != kH_XuatMuonTra_CT.Id);
                        if (findBysID != null)
                        {
                        ...
                        if (dM_SanPham != null)
                        {
                            kH_XuatMuonTra_CT.sDM_DonViTinh_Id = dM_SanPham.sDM_DonViTinh_Id_Cap1;
                            kH_XuatMuonTra_CT.rSoLuong = 1;
                            kH_XuatMuonTra_CT.rDonGia = dM_SanPham.rDonGia_Cap1;
                            kH_XuatMuonTra_CT.rThanhTien = dM_SanPham.rDonGia_Cap1;
                            ...
                            if (!kH_XuatMuonTra_CT.IsLoaded && kH_XuatMuonTra_CT.MTEntityState == Enummation.MTEntityState.Add)
                            {
                                var phukiensCuaSanPham = (List<DM_PhuKien>)DBContext.GetRep2<DM_PhuKienRepository>()
                                                                ...
                                                                , where: $"sDM_SanPham_Id='{kH_XuatMuonTra_CT.sDM_SanPham_Id.Value}'"
                                                                , orderBy: "SortOrder");
                                List<KH_XuatMuonTra_CT_PhuKien> phukienCuaKeHoachNhapMoi = new List<KH_XuatMuonTra_CT_PhuKien>();
                                if (phukiensCuaSanPham != null)
                                {
                                    foreach (var item in phukiensCuaSanPham)
                                    {
                                        phukienCuaKeHoachNhapMoi.Add(new KH_XuatMuonTra_CT_PhuKien
                                        {
                                            Id = Guid.NewGuid(),
                                            rSoLuongPhuKienTren1SP = item.rSoLuong,
                                            rSoLuong = kH_XuatMuonTra_CT.rSoLuong * item.rSoLuong,
                                            ...
                                            rThanhTien = kH_XuatMuonTra_CT.rSoLuong * item.rSoLuong * item.rDonGia,
                                            sDM_SanPham_Id = item.sDM_SanPham_Id
                                        });
                                    }
                                }
                                kH_XuatMuonTra_CT.kH_XuatMuonTra_CT_PhuKiens = phukienCuaKeHoachNhapMoi;
                            }
                        }
                        ...
                        {
                            mGridControl.FuncCustomRecordBeforeAddRow(kH_XuatMuonTra_CT, mGridControl);
                        }
                    }
                    if (e.Column.FieldName == "sDM_DonViTinh_Id")
                    {
                    ...
                    {
                        kH_XuatMuonTra_CT.rThanhTien = kH_XuatMuonTra_CT.rSoLuong * kH_XuatMuonTra_CT.rDonGia;
                    }
                    if (e.Column.FieldName == "sSerial")
                    {
                        ...
                        if (objsSerial != null)
                        {
                            ...
                            {
                                kH_XuatMuonTra_CT.rSoLuong = arrSerial.Length;
                            }
                        }
                    }
                    ...
                    if (e.Column.FieldName == "iThoiGianBaoHanh" || e.Column.FieldName == "iDonViThoiGianBaoHanh")
                    {
                        ...
                        switch (kH_XuatMuonTra_CT.iDonViThoiGianBaoHanh)
                        {
                            case (int)MT.Data.iDonViThoiGianHieuLuc.Ngay:
                                dHanBaoHanhDen =now.AddDays(kH_XuatMuonTra_CT.iThoiGianBaoHanh).AddDays(1);
                                break;

                            case (int)MT.Data.iDonViThoiGianHieuLuc.Thang:
                                dHanBaoHanhDen = now.AddMonths(kH_XuatMuonTra_CT.iThoiGianBaoHanh).AddDays(1);
                                break;

                            case (int)MT.Data.iDonViThoiGianHieuLuc.Quy:
                                dHanBaoHanhDen = now.AddYears(kH_XuatMuonTra_CT.iThoiGianBaoHanh * 3).AddDays(1);
                                break;

                            case (int)MT.Data.iDonViThoiGianHieuLuc.Nam:
                                dHanBaoHanhDen = now.AddYears(kH_XuatMuonTra_CT.iThoiGianBaoHanh).AddDays(1);
                                break;
                        }
                        kH_XuatMuonTra_CT.dHanBaoHanhDen = dHanBaoHanhDen;
                    }
                    ...
                    //if (e.Column.FieldName == "rSoLuong" || e.Column.FieldName == "rDonGia")
                    //{
                    //    var kH_XuatMuonTra_CT = mGridControl.GetRecordByRowSelected<KH_XuatMuonTra_CT>();
                    //    mGridControl.FirstView.SetRowCellValue(e.RowHandle, "rThanhTien", kH_XuatMuonTra_CT.rSoLuong * kH_XuatMuonTra_CT.rDonGia);
                    //}
        ...
        protected override void BindingDataIntoFormSucces()
        {
            ...
            switch (this.FormAction)
            {
                ...
                case (int)MTControl.FormAction.View:
                    ...
                    ucThamChieuPhieuNhapMoi.sTenBangCanCu = "KH_XuatMuonTra";
        ...
        {
            var currentData = (KH_XuatMuonTra)base.PrepareDataBeforeBindDataForm();
