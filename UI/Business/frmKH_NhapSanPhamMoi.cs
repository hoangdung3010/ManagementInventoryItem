...
namespace FormUI
{
    public partial class frmKH_NhapSanPhamMoi : FormUI.MFormList
    {
        #region"Property"
        ...
        private void InitGrid()
        {
            ...
            grd.IsRemoteFilter = true;

            grd.FirstView.OptionsCustomization.AllowSort = true;
            ...
            var colMaster=this.gridMaster.AddColumnText("sDM_DonVi_Id_DonViXayDungKH_Ten", "Đơn vị lập", "Đơn vị lập", 180, isFixWidth: true);
            ...
            this.gridMaster.AddColumnText("dNgayLap", "Ngày lập", 80,dataType:DataTypeColumn.DateEdit);


            this.gridMaster.AddColumnText("sSoHopDong", "Số hợp đồng", 100);
            this.gridMaster.AddColumnText("dNgayHopDong", "Ngày hợp đồng", 80, dataType: DataTypeColumn.DateEdit);
            this.gridMaster.AddColumnText("iThoiGianHieuLuc", "Thời gian hiệu lực", 80, dataType: DataTypeColumn.SpinEdit);
            colMaster=this.gridMaster.AddColumnText("sDM_NhaCC_Id_Ten", "Nhà cung cấp", 180);
            colMaster.FilterEditor = new ConfigFilterEditor { DataType = DataTypeColumn.LookUpEdit, DisplayText = "sTenNCC", TableName = "DM_NhaCC" };
            colMaster.OptionsColumn.AllowSort = DevExpress.Utils.DefaultBoolean.True;

             colMaster =this.gridMaster.AddColumnText("sDM_HinhThucLCNT_Id_Ten", "Hình thức LCNT","Hình thức lựa chọn thầu", 150);
            colMaster.FilterEditor = new ConfigFilterEditor { DataType = DataTypeColumn.LookUpEdit, DisplayText = "sTenHinhThucLCNT", TableName = "DM_HinhThucLCNT" };

            colMaster=this.gridMaster.AddColumnText("sDM_NguonVon_Id_Ten", "Nguồn vốn", 150);
            colMaster.FilterEditor = new ConfigFilterEditor { DataType = DataTypeColumn.LookUpEdit, DisplayText = "sTenNguonVon", TableName = "DM_NguonVon" };
            ...
            colMaster=this.gridMaster.AddColumnText("sDM_DonVi_Id_DonViNhap_Ten", "Đơn vị nhập", 200);
            colMaster.FilterEditor = new ConfigFilterEditor { DataType = DataTypeColumn.TreeLookUpEdit, DisplayText = "sTenDonVi", TableName = "DM_DonVi" };
            ...
            colMaster=this.gridMaster.AddColumnText("iTrangThaiDuyet", "Trạng thái duyệt", 100,enumName: "iTrangThaiDuyetKHNM");

            colMaster.FilterEditor = new ConfigFilterEditor { DataType = DataTypeColumn.ComboBox };

            colMaster=this.gridMaster.AddColumnText("iTrangThaiThucHienKHNM", "Trạng thái thực hiện", 100, enumName: "iTrangThaiThucHienKHNM");

            colMaster.FilterEditor = new ConfigFilterEditor { DataType = DataTypeColumn.ComboBox };
            //Sản phẩm
            grdSanPham.TableName = "KH_NhapSanPhamMoi_CT";
            grdSanPham.ViewName = "View_KH_NhapSanPhamMoi_CT";
            ...
            grdPhuKienDetail.TableName = "KH_NhapSanPhamMoi_CT_PhuKien";
            grdPhuKienDetail.TableName = "View_KH_NhapSanPhamMoi_CT_PhuKien";
        ...
        bool CheckPermissionAproveOrReject(MT.Data.iTrangThaiDuyetKHNM iTrangThaiDuyetKHNM)
        {
            var datas= gridMaster.GetListRecordByRowsSelected<MT.Data.BO.KH_NhapSanPhamMoi>();
            ...
            foreach (var item in datas)
            {
                ...
                //Nếu kế hoạch nhập mới đã phát sinh phiếu nhập mới thì không cho sửa
                List<string> phieuNhapMoi_sMas = DBContext.GetRep2<KH_NhapSanPhamMoiRepository>().GetPhieuNhapMoisByKHNM(item.Id);
                if (phieuNhapMoi_sMas.Count > 0)
                {
                    MMessage.ShowWarning($"Bạn không được phép {msg}. Do kế hoạch nhập mới <{item.sMa}> đã phát sinh các phiếu nhập mới <{string.Join(", ", phieuNhapMoi_sMas)}>.");
                    return false;
                }
            }
        ...
        protected override void ProcessItemClick(int tag, object sender)
        {
            switch (tag)
            {
                case (int)MCommandName.Edit:
                    //1. Kiểm tra nếu phiếu đã duyệt rồi thì không cho sửa
                    //2. Nếu kế hoạch đã phát sinh dữ liệu rồi thì cũng không cho sửa 
                    var selectedItem = gridMaster.GetRecordByRowSelected<MT.Data.BO.KH_NhapSanPhamMoi>();
                    //Nếu kế hoạch nhập mới đã phát sinh phiếu nhập mới thì không cho sửa
                    List<string> phieuNhapMoi_sMas = DBContext.GetRep2<KH_NhapSanPhamMoiRepository>().GetPhieuNhapMoisByKHNM(selectedItem.Id);
                    if (phieuNhapMoi_sMas.Count>0)
                    {
                        MMessage.ShowWarning($"Bạn không được phép sửa. Do kế hoạch nhập mới <{selectedItem.sMa}> đã phát sinh các phiếu nhập mới <{string.Join(", ", phieuNhapMoi_sMas)}>.");
                        return;
                    }
                    break;
            }
            ...
            switch (tag)
            {
                    case (int)MCommandName.Approve:
                        ...
                        {
                            var datasApprove = gridMaster.GetListRecordByRowsSelected<MT.Data.BO.KH_NhapSanPhamMoi>();
                            ...
                            var frmKH_NhapSanPhamMoi_PheDuyet = new frmKH_NhapSanPhamMoi_PheDuyet(this.RefreshData, (int)MT.Data.iTrangThaiDuyetKHNM.DongYDuyet, datasApprove);
                            frmKH_NhapSanPhamMoi_PheDuyet.ShowDialog();
                            frmKH_NhapSanPhamMoi_PheDuyet.Dispose();
                        }
                        break;
                    case (int)MCommandName.Reject:
                        if (CheckPermissionAproveOrReject(MT.Data.iTrangThaiDuyetKHNM.TuChoiDuyet))
                        {
                            var datasReject = gridMaster.GetListRecordByRowsSelected<MT.Data.BO.KH_NhapSanPhamMoi>();
                            ...
                            var frmKH_NhapSanPhamMoi_TuChoi = new frmKH_NhapSanPhamMoi_PheDuyet(this.RefreshData, (int)MT.Data.iTrangThaiDuyetKHNM.TuChoiDuyet, datasReject);
        ...
        protected override void CustomLoadFinishDetailByMaster(MGridControl gridControl, IList data)
        {
            ...
            if (gridControl.TableName == "KH_NhapSanPhamMoi_CT")
            {
        ...
        private void LoadMasterData(object sender, EventArgs e)
        {
            try
            {
                ...
                PagingList.SetWhere($@" dNgayLap BETWEEN '{drKyBaoCao.FromDate.Value.StartDate()}' 
        ...
        private void backgroundWorkerPhuKien_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                e.Result = DBContext.GetRep<KH_NhapSanPhamMoi_CTRepository>().GetDetailsByMasterID2("View_KH_NhapSanPhamMoi_CT_PhuKien",
                        "KH_NhapSanPhamMoi_CT_PhuKien", e.Argument);
            }
            finally
            {
